...
-- -----------------------------------------------------
-- Table `ATO_authenticator`.`application`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ATO_authenticator`.`application` ;

CREATE TABLE IF NOT EXISTS `ATO_authenticator`.`application` (
...
CREATE TABLE IF NOT EXISTS `ATO_authenticator`.`application_priviliges` (
  ...
  CONSTRAINT `APPid_AppPrivs`
    FOREIGN KEY (`APPid`)
    REFERENCES `ATO_authenticator`.`application` (`APPid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
...
START TRANSACTION;
USE `ATO_authenticator`;
INSERT INTO `ATO_authenticator`.`priviliges` (`PRIVid`, `name`, `desc`, `notes`) VALUES (DEFAULT, 'Order of Queensland', 'Recipients earn the right to play minesweeper, and drive really badly', 'god damn these people drive poorly');
INSERT INTO `ATO_authenticator`.`priviliges` (`PRIVid`, `name`, `desc`, `notes`) VALUES (DEFAULT, 'Procrastinator', 'Hard earned gaming time', 'too much gaming time');
...
-- -----------------------------------------------------
-- Data for table `ATO_authenticator`.`application`
-- -----------------------------------------------------
START TRANSACTION;
USE `ATO_authenticator`;
INSERT INTO `ATO_authenticator`.`application` (`APPid`, `appName`, `desc`, `notes`) VALUES (DEFAULT, 'Minesweeper', 'Fun', 'I played this too much as a member of the low socio-economic population in childhood');
INSERT INTO `ATO_authenticator`.`application` (`APPid`, `appName`, `desc`, `notes`) VALUES (DEFAULT, 'Discord', 'High restrictions needed here', 'Probably a security risk');
INSERT INTO `ATO_authenticator`.`application` (`APPid`, `appName`, `desc`, `notes`) VALUES (DEFAULT, 'Portal 2', 'There is no cake', 'There is merely the illusion of cake');
